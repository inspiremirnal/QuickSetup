...
for name in 1 2
do
    multipass launch --name master-${name} -c 2 -m 2G -d 10g --cloud-init ./cloud-config-master.yaml 
done

echo -e "Spinning up worker nodes ..."
for name in 1 2
do
      multipass launch --name worker-${name} -c 2 -m 2G -d 10g --cloud-init ./cloud-config-nodes.yaml
done      
